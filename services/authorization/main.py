...
SERVICE_NAME = os.environ.get("SERVICE_NAME", "authorization-service")
CONFIG = SERVICE_CONFIG[SERVICE_NAME]
TOPIC = CONFIG["topic"]
FAILURE_CODES = ["TIMEOUT", "GATEWAY_ERROR", "ISSUER_DECLINED", "INSUFFICIENT_FUNDS", "SERVICE_ERROR"]
...
def generate_transaction_events() -> list[dict]:
    ...
    parent_span_id = requested["span_id"]
    ...
    try:
        while True:
            for event in generate_transaction_events():
                ...
                print(
                    f"[authorization-service] sent event_id={event['event_id']} "
